import argparse
from rl.networks import MLDense, MLBasicLayer
from rl.policy import PGPolicy
from rl.data.collector import Collector
from rl.data.buffer import ReplayBuffer
from rl.networks import ActorProb
from rl.env import Ant_env
...
def test_ant_pg(args=get_args()):
    ...
    # 创建预处理网路
    preprosses_net1 = MLBasicLayer(args.state_shape, hidden_sizes=args.hidden_size)
    preprosses_net1.summary()
    preprosses_net2 = MLDense(args.state_shape, hidden_sizes=args.hidden_size)
    # 创建actor
    actor1 = ActorProb(
        preprosses_net1, args.action_shape, unbounded=True, conditioned_sigma=False
    )
    actor2 = ActorProb(
        preprosses_net2, args.action_shape, unbounded=True, conditioned_sigma=False
    )
    # 创建策略
    optimizer = tf.optimizers.Adam(learning_rate=args.learning_rate)  # 优化器
    policy = PGPolicy(
        model=actor1, optim=optimizer, evn=ant0, discount_factor=args.gamma
    )
