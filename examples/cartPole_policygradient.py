...
from src.networks.discrete import Actor, Critic
from src.data.buffer import ReplayBuffer
from src.data.collector import Collector
from src.policy import PolicyGradient
import argparse
import numpy as np
...
def test_ant_pg(args=get_args()):
    ...
    preprosses_net1 = MLBasicLayer(args.state_shape,
                                   ...
                                   softmax=False)
    preprosses_net2 = MLBasicLayer(args.state_shape,
                                   hidden_sizes=args.hidden_size,
                                   activations=args.activations,
                                   softmax=True)
    # 创建actor
    actor = Actor(preprosses_net=preprosses_net1,
    ...
    policy = PolicyGradient(env=agent,
                            actor=actor,
    ...
    trainer = Online(buffer=buffer,
                     ...
                     rollout_times=1)
    trainer.train(method='offpolicy',
                  pre_process_funs=[
                      policy.compute_episodic_return,
                      policy.return_normalization
                  ])
