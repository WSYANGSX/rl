...
class Reinforce(BasePolicy):
    ...
    def __init__(
            ...
            critic: Optional[keras.Model] = None,  # 基线价值函数（需要与动作无关，所以使用价值函数）
            critic_optim: Optional[
                tf.optimizers.Optimizer] = None,  # critic优化器
            **kwargs: Any) -> None:
        ...
        if self._base_line:
            for flow in input_data:
                for step in range(len(flow['obs'])):
                    obs_step = tf.convert_to_tensor(flow['obs'][step],
                                                    dtype=tf.float32)
                    if obs_step.ndim == 1:
                    ...
                    with tf.GradientTape(persistent=True) as tape:
                        ...
                        actor_loss = -self._gamma**step * flow['returns'][
                            step] * log_prob
                        critic_loss = 1 / 2 * (flow['returns'][step] -
                                               self._critic(obs_step))**2
