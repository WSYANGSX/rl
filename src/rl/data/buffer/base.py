...
from rl.data.convert import to_hdf5, from_hdf5
...
class ReplayBuffer:
    ...
    def add(self, obj: Union[Flow, dict, Sequence[Union[dict, Flow]]]) -> None:
        if self._size < self._capacity:
            if isinstance(obj, (Flow, dict)):
                if self._meta.empty():
                    ...
                    assert sorted(self._meta.keys()) == sorted(
                        obj.keys()
                    ), "Added flow has different attributes!"
            ...
            print("The replaybuffer element is full, del element from the top!")

    def __getitem__(
        self, index: Union[int, List[int], Tuple[int], np.ndarray, slice]
    ) -> Flow:
        new_flow = Flow()
        ...
        if operator.eq(avaiable_indices, [0]) and index == 0:
            ...
            assert set(indices).issubset(
                avaiable_indices
            ), f"index is out of avaiable_indices!"
            for i in indices:
    ...
    def _sample_indices(self, flow_size: int) -> np.ndarray:
        assert (
            flow_size >= 0 and flow_size <= self._size
        ), f"0 =< batch_size <= self._size!"
        if flow_size == 0:
    ...
    def sample(
        self, indices: Union[int, List[int], Tuple[int], np.ndarray, slice]
    ) -> Flow:
    ...
    def eposide_sample(self, eposide_num: int = 1) -> list[Flow]:
        ...
        if len(done) < eposide_num == 0:
            ...
            extract_end_pos = np.random.choice(done, eposide_num, replace=False)
    ...
    def pop(
        self, index: Union[int, List[int], Tuple[int], np.ndarray, slice] | None = 0
    ) -> Flow:
        new_flow = self[index]
        if isinstance(index, int):
            ...
            for key, value in self._meta.items():
                if isinstance(value, np.ndarray):
                    ...
                    self._meta[key] = [
                        i for num, i in enumerate(self._meta[key]) if num not in index
                    ]
    ...
    def __deepcopy__(self, memo=None, _nil=[]) -> "Flow":
        ...
        for key in self.__dict__.keys():
            new_replaybuffer.__setattr__(
                deepcopy(key, memo), deepcopy(self.__dict__[key], memo)
            )
