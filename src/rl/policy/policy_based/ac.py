...
class ACPolicy(BasePolicy):
    ...
    def process_fn(
        self, flow: Union[Flow, ReplayBuffer], index: int | None = None
    ) -> float:
        """
        ...
        if isinstance(flow, ReplayBuffer):
            ...
            for i in range(len(flow["get"])):
                if i == 0:
                    flow["get"][len(flow["get"]) - i - 1] = flow["rew"][
                        len(flow["get"]) - i - 1
                    ]
                else:
                    flow["get"][len(flow["get"]) - i - 1] = (
                        flow["rew"][len(flow["get"]) - i - 1]
    ...
    def learn(
        ...
        with tf.GradientTape() as tape1:
            ...
            actor_loss = -tf.math.reduce_sum(action_log_probs * flow["advantage"])
            critic_loss = huber_loss(flow["get"], self._critic(flow["obs"]))
            loss = actor_loss + critic_loss
        grads = tape1.gradient(loss, self._critic.trainable_variables)

    def update(
        self, flow: Union[Flow, ReplayBuffer], step: Optional[int] = None
    ) -> None:  # 可以应用与online和offline trainer
