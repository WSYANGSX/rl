...
class Critic(keras.Model):
    ...
    def call(
        self, obs: np.ndarray | tf.Tensor, action: Optional[tf.Tensor] = None
    ) -> tf.Tensor:
        if self._qcritic:
...
class ActorProb(keras.Model):
        ...
        **kwargs,
        ...
        if self._c_sigma:  # 用于SAC
            ...
            self.sigma_variable = tf.Variable(
                tf.zeros([self._output_dim]), trainable=self._trainable
            )  # sigma为可训练张量
        self._max_action = max_action
    ...
    def call(self, obs: np.ndarray | tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        if self._c_sigma:
            sigma = tf.exp(tf.clip_by_value(self.sigma(logits), SIGMA_MIN, SIGMA_MAX))
        else:
